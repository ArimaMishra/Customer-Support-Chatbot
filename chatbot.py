...
class SupportBot:
    ...
    exit_commands = ("exit", "quit", "bye", "goodbye", "farewell", "pause")

    def __init__(self):
        self.support_responses = {
            'ask_about_product': r'.*product.*',
            'technical_support': r'.*(technical|tech).*support.*',
            'about_returns': r'.*(return|refund).*',
    ...
    def chat(self):
        ...
        while not self.make_exit(reply):
            bot_reply = self.match_reply(reply)
            reply = input(bot_reply).lower()
